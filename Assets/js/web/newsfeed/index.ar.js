...
function BindGridArticles(data) {
	...
	$.each(data, function (k, v) {
		...
		template += '				<img class="blur-up lazyload" data-src="' + v.Cover + '" src="' + v.Cover + '" alt="' + v.Title + '">';
		template += '			</a>';
		template += '			<h2 class="h3 text-right">';
		template += '				<a href="/' + lang + '/news/' + v.Slug + '">' + v.Title + '</a>';
		template += '			</h2>';
		template += '			<ul class="publish-detail text-right">';
		template += '				<li>';
		template += '					 <time datetime="' + v.Date + '">' + v.Date + '</time> <i class="icon anm anm-clock-r"></i>';
		template += '				</li>';
		template += '			</ul>';
		template += '			<div class="rte text-right">';
		template += '				<p>' + v.Description + '...</p>';
		template += '			</div>';
		template += '			<p>';
		template += '				<a href="/' + lang + '/news/' + v.Slug + '" class="btn btn-secondary btn--small" title="Read more">';
		template += '					<i class="fa fa-caret-left" aria-hidden="true"></i>';
		template += '					قراءة المزيد';
		template += '				</a>';
		template += '			</p>';
		template += '		</div>';
	...
	if ($('#ArticleContainers').html().length == 0) {
		...
		$('#ArticleContainers').html('<div class="alert alert-dark" title="No Records Found!">لم يتم العثور على سجلات!</div>');
	}
}
