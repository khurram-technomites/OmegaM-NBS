        ...
        "columns": [
                ...
                "render": function (data, type, row) {
                    ...
                    if (Published == "FALSE") {
                        return `<label class="checkbox checkbox-lg checkbox-inline">
										<input type="checkbox" value="${row.ID}" id="chk${row.ID}" name="chkCar" onchange="PropertySelected(this,${row.ID})">
										<span></span>
									</label>`;
                    } else {
                        return '';
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    ...
                    console.log(split[3].toString())
                    var Verified = split[7] == "True" ? "Verified" : "";
                    var Premium = split[6] == "True" ? "Premium" : "";
                    var Verifiedclass = split[7] == "True" ? "" : "hidden";
                    var Premiumclass = split[6] == "True" ? "" : "hidden";
                    if (split[3].toString() == "True") {
                        return '<div class="d-flex align-items-center">' +
                            '<div class="symbol symbol-50 flex-shrink-0 mr-4">' +
                            '<div class="symbol-label" style="background-image: url(\'' + split[0] + '\')"></div>' +
                            '</div>' +
                            '<div class="car-name" title="' + split[1] + '">' +
                            `<div class="preview" onclick="CopyToClipboard(this,'${split[4]}')"><i class="fa fa-link"></i> | Copy URL to Clipboard</div></br>` +
                            '<a href="javascript:;" class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">' + split[1] + '</a>' +
                            '<span class="text-muted font-weight-bold d-block sku">' + split[2] + `<small>(Call Clicks: ${split[5] ? split[5] : 0})</small>` + '</span>' +
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    if (data == "True") {
                        return "<label class='switch d-flex justify-content-center switch'>" +
                            "<input type ='checkbox' id='toggle" + row.ID + "' checked onClick='ToggleIsSold(" + row.ID + ", " + true + ")'>" +
                            "<span class='slider round'></span>" +
                            "</label>" 
                           
                    } else {
                        return "<label class='switch d-flex justify-content-center switch'>" +
                            "<input type ='checkbox' id='toggle" + row.ID + "' onClick='ToggleIsSold(" + row.ID + ")' , " + false + ">" +
                            "<span class='slider round'></span>" +
                            "</label>"
                    }
                }
            ...
            //        var active = data ? data.toString().toUpperCase() : "FALSE";
            //        var status = {
            //            "TRUE": {
            //                'title': 'Active',
            //                'class': ' label-light-success'
            ...
            //},
            {
                ...
                "render": function (data, type, row) {
                    ...
                    var isActive = row.IsPublished ? row.IsPublished.toString().toUpperCase() : "FALSE";
                    var status = {
                        "TRUE": {
                            'title': 'UnPublish',
                    ...
                    var active = row.IsActive ? row.IsActive.toString().toUpperCase() : "FALSE";
                    var Propstatus = {
                        "TRUE": {
                            'title': 'Deactivate',
                            'icon': 'fa-times-circle',
                    ...
                    var actions = '';
                    actions += '<a class="btn btn-bg-secondary btn-icon btn-sm mr-1" href="/Vendor/Property/Edit/' + row.ID + '">' +
                        '<i class="fa fa-pen"></i>' +
                        '</a> ' +
                    ...
                    if (typeof status[isActive] === 'undefined') {
                        actions += '<button type="button" class="btn btn-outline-success btn-sm mr-1" onclick="Publish(this, ' + row.ID + ')">' +
                            '<i class="fa fa-check-circle" aria-hidden="true"></i> Publish' +
                            '</button>';
                    }
                    else {
                        actions += '<button type="button" class="btn btn-sm mr-1' + status[isActive].class + '" onclick="Publish(this, ' + row.ID + ')">' +
