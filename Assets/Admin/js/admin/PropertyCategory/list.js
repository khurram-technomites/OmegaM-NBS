            ...
            columnDefs: [
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        return `<div class="d-flex align-items-center">
						        ...
						        <div class="symbol symbol-50 flex-shrink-0 mr-4">
						            <div class="symbol-label Thumbnail" style="background-image: url(${category[1]})">
						            <img class="Icon" src="${category[0]}">
						        </div>
						        </div>
						        <div>
						            <a href="#" class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">${category[2]}</a>
						        </div>
						    </div>`;
                    },
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (newData[0] == "True" && newData[1] == "True") {
                            action = "Sale/Rent"
                        }

                        else if (newData[0] == "True") {
                            action = "Sale"
                        }
                        ...
                        return `<span class="label label-lg font-weight-bold label-inline">${action}</span>`;
                    },
                },
                {
                    targets: 4,
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/PropertyCategory/Edit/' + data[1] + '\',true)">' +
                            '<i class="fa fa-pen"></i>' +
                            '</button> ' +
                            '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/PropertyCategory/Details/' + data[1] + '\')" title="View">' +
                        ...
                        if (typeof status[isActive] === 'undefined') {
                            //if (full[4] && full[4].toUpperCase() === "TRUE") {
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/PropertyCategory/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/PropertyCategory/Activate/' + record,
                type: 'Get',
