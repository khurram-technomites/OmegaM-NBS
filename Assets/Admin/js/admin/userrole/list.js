            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        return '<a href="/Admin/UserRole/Privileges/' + data + '" class="btn btn-secondary btn-sm mr-1" >' +
                            '<i class="flaticon2-pen"></i> Privileges </a>'
                    },
                },
                {
                    targets: -1,
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (full[1] === "Admin") {
                            actions += '<button type="button" class="btn btn-secondary btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/UserRole/Details/' + data[1] + '\')" title="View">' +
                            ...
                            actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/UserRole/Edit/' + data[1] + '\',true)">' +
                                '<i class="fa fa-pen"></i>' +
                                '</button> ' +
                                '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/UserRole/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/UserRole/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/UserRole/Activate/' + record,
                type: 'Get',
...
function addRow(row) {
    ...
    //    row.RoleName,
    //    '<a href="/Admin/UserRole/Privileges/' + row.ID + '" class="btn btn-secondary btn-sm mr-1">' +
    //    '<i class="flaticon2-pen"></i> Privileges' +
    //    '</a>',
    //    row.IsActive,
    //    row.IsActive + ',' + row.ID,
    //]).draw(true);
