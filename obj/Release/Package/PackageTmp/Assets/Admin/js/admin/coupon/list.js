		...
		table1 = table.DataTable({
			...
			columnDefs: [
						...
						if (coupon.length > 2) {
							return '<div class="d-flex align-items-center">' +
										...
										'<div>' +
											...
											'<span class="text-muted font-weight-bold d-block"><i class="fa fa-hashtag fa-sm"></i> ' + coupon[1] + ' <i class="fa fa-circle fa-separator"></i> ' + coupon[3] + (coupon[2] == "FixedAmount" ? ' <i class="fa fa-credit-card fa-sm"></i>' : ' <i class="fa fa-percent fa-sm"></i>') + '</span>' +
										'</div>' +
									'</div>';
						} else {
							return '<span>-</span>';
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						console.log(data);
						return '<button type="button" class="btn btn-bg-secondary btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/CouponCategory/Index/' + data + '\',true)">' +
				...
				},
				 {
				 	...
				 	render: function (data, type, full, meta) {
				 		...
				 		if (data) {
				 			var actions = '<button type="button" class="btn btn-outline-success btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Coupon/SetCoupons/' + data + '\',true)">' +
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						...
						actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Coupon/Edit/' + data[1] + '\',true)">' +
										'<i class="fa fa-pen"></i>' +
									'</button> ' +
									'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Coupon/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/Coupon/Delete/' + record,
				type: 'POST',
				data: {
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/Coupon/Activate/' + record,
				type: 'Get',
