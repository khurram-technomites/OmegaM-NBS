...
$(document).ready(function () {
    ...
    $('.checkbox-list').on('change', 'input[type=checkbox]', function (event) {
        ...
        if (checked) {
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/Property/AssignFeature?PropId=" + propID + "&FeatureId=" + id,
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/Property/RemoveFeature?PropId=" + propID + "&FeatureId=" + id,
                data: { "__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val() },
                success: function (data) {
                    toastr.success(data.message);
                }
    ...
    $('#kt_image_Property_image #Image').change(function () {
        ...
        if (files.length > 0) {
            ...
            $.ajax({
                url: "/Admin/Property/Thumbnail/" + $("#ID").val(),
                type: "POST",
                processData: false,
                contentType: false,
                data: data,
    ...
    /*   $('#kt_Video_car_Video #Video').change(function () {
           ...
           if (files.length > 0) {
               ...
               $.ajax({
                   url: "/Admin/Property/Video/" + GetURLParameter(),
                   type: "POST",
                   processData: false,
                   contentType: false,
                   data: data,
   ...
   */
    ...
    $('input[name=GalleryImages]').change(function () {
        ...
        if (files.length > 0) {
            ...
            $.ajax({
                url: "/Admin/Property/CreateImage/" + $("#ID").val(),
                type: "POST",
                processData: false,
                contentType: false,
                data: data,
                    ...
                    if (response.success) {
                        $(response.data).each(function (k, v) {
                            $('.Property-gallery-images').append('<div class="symbol symbol-70 flex-shrink-0 mr-5 mb-3">' +
                                ...
                                '</span>' +
                                '<div class="symbol-label property-image" data-id=' + TotalImage + 1 +' style="background-image: url(\'' + v.Value + '\')"></div>' +
                                '</div>');
                        });
    ...
    $('input[name=FloorImages]').change(function () {
        ...
        if (files.length > 0) {
            ...
            $.ajax({
                url: "/Admin/Property/CreateFloorImage/" + $("#ID").val(),
                type: "POST",
                processData: false,
                contentType: false,
                data: data,
...
function SaveNewCarInspection() {
    ...
    $.ajax({
        url: '/Admin/PropertyInspection/CreateInspection/',
        type: 'POST',
        processData: false,
        contentType: false,
        data: data,
...
function DeleteInspection(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $(this).addClass('spinner spinner-dark spinner-right');
            $.ajax({
                url: '/Admin/PropertyInspection/DeletePropertyInspection/' + record,
                type: 'POST',
                data: {
                    "__RequestVerificationToken":
                        $("input[name=__RequestVerificationToken]").val()
...
function bindButton() {
    ...
    $('#PropertyLocationForm').submit(function () {
        ...
        $.ajax({
            url: '/Admin/Property/UpdateLocation/',
            type: 'POST',
            data:
            {
                "__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val(),
...
function DeleteGalleryImage(elem, record) {
    ...
    $(elem).find('i').hide();
    $.ajax({
        url: '/Admin/Property/DeleteImage/' + record,
        type: 'POST',
        data: {
            "__RequestVerificationToken":
                $("input[name=__RequestVerificationToken]").val()
...
function DeleteFloorImage(elem, record) {
    ...
    $(elem).find('i').hide();
    $.ajax({
        url: '/Admin/Property/DeleteFloorImage/' + record,
        type: 'POST',
        data: {
            "__RequestVerificationToken":
                $("input[name=__RequestVerificationToken]").val()
