            ...
            columnDefs: [{
                ...
                render: function (data, type, full, meta) {
                    ...
                    if (IsApproved == '' || IsApproved == 'null') {
                        actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/NearByPlacesCategory/Edit/' + data[1] + '\',true)">' +
                            '<i class="fa fa-pen"></i>' +
                            '</button> ' +
                            '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/NearByPlacesCategory/Details/' + data[1] + '\')" title="View">' +
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    return '<div class="d-flex align-items-center">' +
                        ...
                        '<div class="symbol-label symbol-label-color-size bg-transparent" style="background-repeat: no-repeat; background-image: url(\'' + category[0] + '\')"></div>' +
                        '</div>' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/NearByPlacesCategory/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/NearByPlacesCategory/Activate/' + record,
                type: 'Get',
...
function Approval(element, record, value) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/NearByPlacesCategory/Approve/',
