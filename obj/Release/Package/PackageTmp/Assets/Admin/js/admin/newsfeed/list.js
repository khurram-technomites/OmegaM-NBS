            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        actions += '<a  class="btn btn-bg-secondary btn-icon btn-sm mr-1" href="/Admin/NewsFeed/Edit/' + data[1] + '">' +
                            '<i class="fa fa-pen"></i>' +
                            '</a> ' +
                            '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/NewsFeed/Details/' + data[1] + '\')" title="View">' +
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        return `<div class="d-flex align-items-center">
								...
								<div>
                                    ...
                                    ${title[2] != '' ? `<span class="text-muted font-weight-bold d-block" style="color:#008bb9 !important">Module: ${title[2]}</span>` : ``}
								</div>
							</div>`;
                    },
                },
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/NewsFeed/Delete/' + record,
                type: 'POST',
                data: {
                ...
                success: function (result) {
                    if (result.success != undefined) {
                        if (result.success) {
                            ...
                            toastr.success('NewsFeed deleted successfully ...');
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/NewsFeed/Activate/' + record,
                type: 'Get',
...
function addRow(row) {
    table1.row.add($('<tr>' +
        ...
        '<td>' + row.BannerImage + '|' + row.Title + '|' + row.Module + '</td>' +
        '<td>' + row.IsActive + '</td>' +
