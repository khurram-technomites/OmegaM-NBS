                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (IsApproved == '' || IsApproved == 'null') {
                            actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/CarMake/Edit/' + data[1] + '\',true)">' +
                                '<i class="fa fa-pen"></i>' +
                                '</button> ' +
                                '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/CarMake/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/CarMake/Delete/' + record,
                type: 'POST',
                data: {
                ...
                success: function (result) {
                    if (result.success != undefined) {
                        if (result.success) {
                            ...
                            toastr.success('Make deleted successfully ...');
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/CarMake/Activate/' + record,
                type: 'Get',
...
function addRow(row) {
    table1.row.add($('<tr>' +
        ...
        '<td>' + row.Name + '</td>' +
        '<td>' + row.IsActive + '</td>' +
        '<td nowrap="nowrap">' + row.IsActive + ',' + row.ID + ',' + row.IsApproved + '</td>' +
        '</tr>'
    )).draw();
}