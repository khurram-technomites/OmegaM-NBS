        ...
        table1 = table.DataTable({
            ...
            columnDefs: [{
                ...
                render: function (data, type, full, meta) {
                    ...
                    actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Services/Edit/' + data[1] + '\',true)">' +
                        '<i class="fa fa-pen"></i>' +
                        '</button> ' +
                        '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Services/Details/' + data[1] + '\')" title="View">' +
...

            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    return '<div class="d-flex align-items-center">' +
                        ...
                        '<div class="symbol-label" style="background-image: url(\'' + category[0].replace("~", "") + '\')"></div>' +
                        '</div>' +
                        '<div>' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/Services/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/Services/Activate/' + record,
                type: 'Get',
                success: function (response) {
                    if (response.success) {
