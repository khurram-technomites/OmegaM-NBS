            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        data = data.split(',');
                        var actions = '';
                        actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="myFunctionDetails(); OpenModelPopup(this,\'/Admin/Vendor/Details/' + data[1] + '\');">' +
                        ...
                        actions += '<button type="button" class="btn btn-outline-success btn-sm mr-1 btnapprove" onclick="Approve(this,' + data[1] + ',true)">' +
                            ...
                            '</button> ';
                        actions += '<button type="button" class="btn btn-outline-danger btn-sm mr-1 btnapprove" onclick="OpenModelPopup(this,\'/Admin/Vendor/Reject/' + data[1] + '/?approvalStatus=false\',true)">' +
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        actions += '<button class="btn btn-bg-secondary" onclick="myFunction(); OpenModelPopup(this,\'/Admin/VendorDocuments/Index/' + data + '\');">' +
...
function Decline(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/Admin/Vendor/Reject/' + record,
                type: 'POST',
                data: {
                    "__RequestVerificationToken":
                        $("input[name=__RequestVerificationToken]").val()
...
function addRow(row) {
    table1.row.add($('<tr>' +
        ...
        '<td>' + row.Logo + '|' + row.Name + '|' + row.VendorCode + '</td>' +
        '<td>' + row.ID + '</td>' +
        '<td>' + row.ID + '</td>' +
        '<td hidden>' + row.Email + '</td>' +
        '<td nowrap="nowrap">' + row.IsApproved + ',' + row.ID + '</td>' +
        '</tr>'
    )).draw();
}