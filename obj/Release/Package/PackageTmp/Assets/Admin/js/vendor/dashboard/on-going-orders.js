			...
			columnDefs: [{
				...
				render: function (data, type, full, meta) {
					...
					if (status && status === "PENDING") {
						...
						actions += '<a class="btn btn-bg-secondary btn-icon btn-sm" href="/Vendor/Order/Details?id=' + data[1] + '">' +
						'<i class="fa fa-arrow-right"></i> ' +
						'</a> ';
					}
					else {
						actions += '<a class="btn btn-bg-secondary btn-sm font-weight-bold" href="/Vendor/Order/Details?id=' + data[1] + '">' +
...
function Reject(element, record) {
	...
	swal.fire({
		...
		if (response.value) {
			...
			$.ajax({
				url: '/Vendor/Dashboard/StatusChange/',
				type: 'POST',
...
function callback(dialog, elem, isedit, response) {
	...
	if (response.success) {
		...
		addRow(response.data);
		jQuery('form', dialog).closest('.modal').find('button[type=submit]').removeClass('spinner spinner-sm spinner-center').attr('disabled', false);
