            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return '<a href="/Vendor/UserRole/Privileges/' + data + '" class="btn btn-secondary btn-sm mr-1" >' +
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        data = data.split(',');
                        if (data[2] == "Lead Manager") {
                            var actions = '';

                            actions +=
                                '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/UserRole/Details/' + data[1] + '\')" title="View">' +
                            ...
                            actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/UserRole/Edit/' + data[1] + '\',true)">' +
                                '<i class="fa fa-pen"></i>' +
                                '</button> ' +
                                '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/UserRole/Details/' + data[1] + '\')" title="View">' +
                                '<i class="fa fa-folder-open"></i>' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Vendor/UserRole/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Vendor/UserRole/Activate/' + record,
                type: 'Get',
