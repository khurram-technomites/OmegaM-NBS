...
jQuery(document).ready(function () {
    $.ajax({
        type: 'Get',
        url: '/Vendor/Notification/LoadNotifications?pageNo=' + PageNo,
...
function FetchNewNotification() {
    $.ajax({
        type: 'Get',
        url: '/Vendor/Notification/GetNotifications/',
        success: function (response) {
            if (response.success) {
...
function getHtmlizeNotification(v) {
    ...
    if (!v.IsRead) {
        //UnRead Messages
        HTMLNotification += '<div class="d-flex align-items-center mb-12" ' + (v.IsSeen ? '' : 'un-seen') + '">';
        HTMLNotification += '<div class="symbol symbol-40 symbol-light-success mr-5 ">';
        HTMLNotification += '<a href="' + (v.Link ? v.Link : 'javascript:;') + '" style="display: block; text-decoration: none; color: inherit">';
        ...
        HTMLNotification += v.Description
        HTMLNotification += '</div>';
        HTMLNotification += '</a>';
        HTMLNotification += '</div>';
        HTMLNotification += '<i class="fa fa-circle notification-read-circle pl-18 cursor-pointer" onclick="IsRead(' + v.ID + ',this);" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="top" float="right" data-content="mark as read"></i>';
        HTMLNotification += '</div>';
    } else {
        HTMLNotification += '<div class="d-flex align-items-center mb-12" >';
        HTMLNotification += '<div class="symbol symbol-40 symbol-light-success mr-5">';
        HTMLNotification += '<a href="' + (v.Link ? v.Link : 'javascript:;') + '" style="display: block; text-decoration: none; color: inherit">';
        HTMLNotification += '<span class="symbol-label">';
        HTMLNotification += '<span class="svg-icon svg-icon-lg svg-icon-success">';
        ...
        HTMLNotification += '</a>';
        HTMLNotification += '</div>';
        HTMLNotification += '<i class="fa fa-circle-notch notification-read-circle pl-18 cursor-pointer" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="top" data-content=""></i>';
        HTMLNotification += '</div>';
    }
...
function IsRead(notificationId, element) {
    $.ajax({
        type: 'POST',
        url: '/Vendor/Notification/MarkNotificationAsRead/?notificationId=' + notificationId,
        success: function (response) {
            if (response.success) {
...
function AllRead(receiverId) {
    if (IsUnReadRecoredExist) {
        $.ajax({
            type: 'POST',
            url: '/Vendor/Notification/NotificationsReadAll/?receiverId=' + receiverId,
            success: function (response) {
                if (response.success) {
...
function IsSeen(receiverId) {
    if (IsUnseenRecoredExist) {
        ...
        $.ajax({
            type: 'POST',
            url: '/Vendor/Notification/MarkNotificationsAsSeen/?receiverId=' + receiverId,
            success: function (response) {
                if (response.success) {
