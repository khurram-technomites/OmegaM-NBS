...
$(document).ready(function () {
	...
	$('#btnCategoryReset').click(function () {
		if (filter.categoryID != null) {
			...
			window.history.pushState('', 'Products', '/shop');
...
function BindFilterCategories(data) {
	...
	$(".sidebar_categories a").on("click", function () {
		...
		if (filter.categoryID != $(this).attr('category')) {
			...
			window.history.pushState('', selectedcategory.Name, '/product-category/' + selectedcategory.Slug);
...
function BindGridProducts(data) {
	...
	$.each(data, function (k, v) {
		...
		template += '		<div class="product-image gallery-product-image">';
		template += '			<!-- start product image -->';
		template += '			<a href="/product/' + v.Slug + '" class="grid-view-item__link">';
		template += '				<!-- image -->';
		template += '				<img class="primary blur-up lazyload img-lazyload gallery-img" data-src="' + v.Thumbnail + '" src="' + v.Thumbnail + '" alt="image" title="product">';
		template += '				<!-- End image -->';
		template += '				<!-- Hover image -->';
		template += '				<img class="hover blur-up lazyload img-lazyload gallery-img-hover" data-src="' + v.Image1 + '" src="' + v.Image1 + '" alt="image" title="product">';
		...
		template += '				<button class="btn btn-addto-cart" data-type="1" onclick="ProductCompareArrayFunction(this, ' + v.ID + ')" id="product-compare-' + v.ID + '" type="button" tabindex="0">Add To Compare</button>';
		template += '			</div>';
		...
		template += '				<a href="/product/' + v.Slug + '">' + v.Title + '</a>';
		template += '			</div>';
	...
	setTimeout(function () { OnErrorImage(); }, 2000);

	ProductsCompareCheckArray();
	...
	if ($('#ProductsContainers').html().length == 0) {
		...
		$('#ProductsContainers').html('<div class="alert alert-dark">No Records Found!</div>');
	}
}
