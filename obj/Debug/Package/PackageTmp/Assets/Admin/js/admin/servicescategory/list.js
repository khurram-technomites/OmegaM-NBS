		...
		table1 = table.DataTable({
			...
			columnDefs: [{
				...
				render: function (data, type, full, meta) {
					...
					actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/ServicesCategory/Edit/' + data[1] + '\',true)">' +
									'<i class="fa fa-pen"></i>' +
								'</button> ' +
								'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/ServicesCategory/Details/' + data[1] + '\')" title="View">' +
...

			{
				...
				render: function (data, type, full, meta) {
					...
					var serviceCategory = data.split('|');
					return '<div class="d-flex align-items-center">' +
								'<div class="symbol symbol-50 flex-shrink-0 mr-4">' +
									'<div class="symbol-label" style="background-image: url(\'' + serviceCategory[0].replace("~", "") + '\')"></div>' +
								'</div>' +
								'<div>' +
									'<a href="#" class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">' + serviceCategory[1] + '</a>' +
									//'<span class="text-muted font-weight-bold d-block">' + serviceCategory[1] + '</span>' +
								'</div>' +
							'</div>';
				},
			},
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/ServicesCategory/Delete/' + record,
				type: 'POST',
				data: {
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/ServicesCategory/Activate/' + record,
				type: 'Get',
